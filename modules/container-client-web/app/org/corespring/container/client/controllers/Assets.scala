...
trait Assets extends Controller with HasContext {
  ...
  private def at(id: String, file: String, notFoundLocally: String => SimpleResult) = Action.async {
    request =>
      logger.trace(s"[at] id: $id file $file")
  ...
  def upload(id: String, file: String) = {
    logger.trace(s"[upload] id: $id file $file")
    hooks.uploadAction(id, file) {
      r => Ok("")
    }
  }
  ...
  def delete(itemId: String, file: String) = Action.async {
    implicit request =>
      logger.trace(s"[delete] itemId: $itemId file $file")
