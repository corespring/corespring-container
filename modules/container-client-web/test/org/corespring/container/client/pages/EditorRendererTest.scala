...
class EditorRendererTest extends Specification with Mockito with NoTimeConversions {
  ...
  trait render extends scope {
    ...
    def prodMode = true
    lazy val html = renderer.render(mainEndpoints, supportingMaterialEndpoints, componentsAndWidgets, clientOptions, bundle, prodMode)
    waitFor(html)
