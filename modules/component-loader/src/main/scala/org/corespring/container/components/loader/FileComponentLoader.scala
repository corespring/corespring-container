    ...
    def apply(org: String, packageJson: JsValue, compRoot: File): LoadedData = {
      ...
      LoadedData(
        ...
        client = loadClient(clientFolder,
          loadLibrarySources(compRoot.getPath, "client/render", createClientName(compRoot.getPath)),
          loadLibrarySources(compRoot.getPath, "client/configure", createClientName(compRoot.getPath))),
