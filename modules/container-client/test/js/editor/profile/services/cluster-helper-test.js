describe('cluster helper', function() {
  ...
  describe('updateClustersFromStandards', function() {
    var clusters;
    beforeEach(function() {
      clusters = [];
    });
    it('should add a cluster', function() {
      sut.updateClustersFromStandards(clusters, [mkStandard('A.B.C', 'some cluster')]);
      expect(clusters).toEqual([mkCluster('some cluster', 'A.B.C', false)]);
    });
    it('should add a cluster twice', function() {
      sut.updateClustersFromStandards(clusters, [
    ...
    it('should add a new cluster as visible, if same clusters are hidden', function() {
      ...
      sut.updateClustersFromStandards(clusters, [mkStandard('A.B.C', 'some cluster'), mkStandard('D.E.F', 'some cluster')]);
      expect(clusters).toEqual([
  ...
  describe('getClustersForUi', function() {
    ...
    it('should only return visible clusters', function() {
      sut.updateClustersFromStandards(clusters, [mkStandard('A.B.C', 'some cluster')]);
      sut.hideCluster(clusters, mkCluster('some cluster', 'A.B.C', false));
      sut.updateClustersFromStandards(clusters, [mkStandard('A.B.C', 'some cluster'), mkStandard('D.E.F', 'that cluster')]);
