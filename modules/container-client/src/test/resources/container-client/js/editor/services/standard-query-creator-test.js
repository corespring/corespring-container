describe('standard query creator', function () {
  ...
  it('should create query with one filter, if subject option is set', function () {
    ...
    expect(result.filters).toEqual([
      {field: 'subject', value: 'important subject'}
    ]);
  });

  it('should create query with two filters, if subject and category option are set', function () {
    var result = sut.createStandardQuery("test", {name: 'important subject'}, {name: 'important category'})
    expect(result.filters).toEqual([
      {field: 'subject', value: 'important subject'},
      {field: 'category', value: 'important category'}
    ]);
  });

  it('should create query with three filters, if subject, category and subCategory option are set', function () {
    var result = sut.createStandardQuery("test", {name: 'important subject'}, {name: 'important category'}, {name: 'important sub-category'})
    expect(result.filters).toEqual([
      {field: 'subject', value: 'important subject'},
      {field: 'category', value: 'important category'},
      {field: 'subCategory', value: 'important sub-category'}
    ]);
  });
  ...
  describe("the first empty option removes all filters after it", function () {
    ...
    it('empty subject option removes all filters', function () {
      var result = sut.createStandardQuery("test", null, {name: 'important category'}, {name: 'important sub-category'})
      expect(result.filters).toBeUndefined();
    });

    it('empty category option removes category and sub-category filter', function () {
      var result = sut.createStandardQuery("test", {name: 'important subject'}, null, {name: 'important sub-category'})
      expect(result.filters).toEqual([
        {field: 'subject', value: 'important subject'}
      ]);
    });
