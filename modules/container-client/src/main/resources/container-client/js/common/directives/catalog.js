angular.module('corespring-common.directives').directive('catalogview', [ '$sce', 'DataQueryService', function($sce, DataQueryService) {
  ...
  return {
    ...
    template: [
      ...
      '      <corespring-player play-mode="editor" player-markup="item.xhtml" player-item="item" player-session="rootModel.session" player-outcomes="outcome"/>',
      ...
      '        <hr/>',
      '        <div id="score">',
      '          <h4>You score: {{score.summary.percentage}}%</h4>',
      '        </div>',
      '      </div>',
      '    </client-side-preview>',
      '  </li>',
