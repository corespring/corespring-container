  ...
  override def submitAnswers(id: String)(block: (SubmitSessionRequest[AnyContent]) => Result): Action[AnyContent] = Action {
    request =>
      ...
      val result = for {
        ...
        SubmitSessionRequest(out, sessionService.save(_, _), request)
      }
      result.map {
        r =>
          block(r)
  ...
  override def save(id: String)(block: (SaveSessionRequest[AnyContent]) => Result): Action[AnyContent] = Action {
    request: Request[AnyContent] =>
      ...
      val result = for {
        ...
        SaveSessionRequest(session, isSecure(request), isComplete(session), sessionService.save(_, _), request)
      }
      result.map {
        r =>
          block(r)
