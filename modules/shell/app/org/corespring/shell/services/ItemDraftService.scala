...
class ItemDraftService(override val collection:MongoCollection) extends MongoService(collection){
  ...
  override def create(data:JsValue) = throw new RuntimeException("use createDraft")

  override def load(id:String)= {
    val draftId : ContainerDraftId = DraftId[ObjectId,ContainerDraftId](id, (itemId,name) => ContainerDraftId(new ObjectId(itemId), name))
    findDraft(draftId)
  }
